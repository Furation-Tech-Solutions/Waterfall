...
export class FeedBackRepositoryImpl implements FeedBackRepository {
  ...
  async createFeedBack(feedBack: FeedBackModel): Promise<Either<ErrorClass, FeedBackEntity>> {
    try {
      const createdFeedBack = await this.feedBackDataSource.create(feedBack); // Use the feedBack data source
      return Right<ErrorClass, FeedBackEntity>(createdFeedBack);
    } catch (error: any) {
  ...
  async getFeedBacks(query: Query): Promise<Either<ErrorClass, FeedBackEntity[]>> {
    try {
      ...
      return Left<ErrorClass, FeedBackEntity[]>(ApiError.customError(400, e.message));
    }
  }
  ...
  // Get the count of feedback entries based on query
