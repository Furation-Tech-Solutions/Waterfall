...
export interface CreatePaymentGatewayUsecase {
  execute: (
...
export class CreatePaymentGateway implements CreatePaymentGatewayUsecase {
  ...
  // Implementation of the 'execute' method defined in the CreatePaymentGatewayUsecase interface
  async execute(
    paymentGatewayData: PaymentGatewayModel
  ): Promise<Either<ErrorClass, PaymentGatewayEntity>> {
    return await this.paymentGatewayRepository.createPaymentGateway(paymentGatewayData); // Call the 'createPaymentGateway' method of the PaymentGatewayRepository and return the result
  }
}
