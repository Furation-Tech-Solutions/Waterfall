...
export interface CheckRealtorByRecoIdUsecase {
  execute: (id: string) => Promise<Either<ErrorClass, RealtorEntity>>;
}

// Implementation of the GetRealtorById use case
export class CheckRealtorByRecoId implements CheckRealtorByRecoIdUsecase {
  ...
  async execute(id: string): Promise<Either<ErrorClass, RealtorEntity>> {
    ...
    return await this.realtorRepository.CheckRecoId(id);
  }
}
