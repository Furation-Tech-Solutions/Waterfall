...
export interface GetAllJobsUsecase {
  execute: (ownerId: string) => Promise<Either<ErrorClass, JobEntity[]>>; // A method named "execute" that returns a Promise of Either type
}
...
export class GetAllJobs implements GetAllJobsUsecase {
  ...
  async execute(ownerId: string): Promise<Either<ErrorClass, JobEntity[]>> {
    return await this.jobRepository.getJobs(ownerId); // Invoke the "getJobs" method of jobRepository and return its result
  }
}
